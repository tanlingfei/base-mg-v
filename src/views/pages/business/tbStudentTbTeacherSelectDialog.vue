  ...
  methods: {
    ...
    init() {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.tbTeacherTable.clearSelection()
        this.resetSearch()
      })
    },
    // 获取数据列表
    refreshList() {
      ...
      tbTeacherApi.getPageList(this.pageNo, this.pageSize, this.searchForm).then(
        response => {
          ...
          if (this.selectData && this.selectData.length <= 10000) {
            ...
            this.$nextTick(function() {
              ...
              if (arr) {
                arr.forEach((row) => {
                  this.$refs.tbTeacherTable.toggleRowSelection(row, true)
                })
              }
            })
            //test end
    ...
    handleSelectionChange(selection, row) {
      if (row) {
        ...
        let isCheckAll = this.$refs.tbTeacherTable.store.states.isAllSelected
        //反选的时候 移除默认
        if (isCheckAll == false) {
          this.$nextTick(function() {
            if (this.selectData && this.selectData.length <= 10000) {
              this.dataList.forEach((item) => {
                this.selectData.forEach((val, index) => {
                  if (val.id == item.id) {
                    this.selectData[index] = { id: '' }
                    console.log('has removed ' + val.teach_name)
                  }
                })
              })
            }
